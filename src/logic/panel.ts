...
export class Panel {
  ...
  constructor(el: Element) {
    ...
    // array of Item
    this.items = [];
    this.DOM.items.forEach((item: any) => this.items.push(new PanelItem(item)));
  ...
  calcImgPosition(imgWrap: { getBoundingClientRect: () => any; parentNode: any }) {
  ...
  positionItemChars(item: { DOM: { imgWrap: { getBoundingClientRect: () => any }; subtitleChars: any[] } }) {
    ...
    for (const [_, char] of item.DOM.subtitleChars.entries()) {
      ...
      const charPosition = {
        x:
          imgRect.left +
          imgRect.width / 2 -
          subtitleCharRect.left -
          subtitleCharRect.width / 2 +
          gsap.utils.random(-150, 150),
        y:
          imgRect.top +
          imgRect.height / 2 -
          subtitleCharRect.top -
          subtitleCharRect.height / 2 +
  ...
  showItemChars(item: any) {
    ...
    item.timelineIn = gsap
      .timeline()
      .addLabel('start', 0)
      .set(
        item.DOM.subtitleText,
        {
          opacity: 1,
        },
        'start'
  ...
  hideItemChars(item: {
    ...
    DOM: { subtitleChars: gsap.TweenTarget; img: string; imgWrap: string; subtitleText: gsap.TweenTarget };
  }) {
    if (this.isOpen) return;

    item.killActiveTimeline();

    item.timelineOut = gsap
      .timeline()
      .addLabel('start', 0)
      .to(
        item.DOM.subtitleChars,
  ...
  openItem(item: {
    DOM: {
      el: { classList: { add: (arg0: string) => void | null } };
      imgWrap: gsap.TweenTarget;
      img: string;
      subtitleHeader: gsap.TweenTarget;
      closeCtrl: gsap.TweenTarget;
      subtitleChars: gsap.TweenTarget;
      titleTexts: gsap.TweenTarget;
    };
  }) {
    if (this.isOpen) return;
    this.isOpen = true;
    ...
    const otherImgWrap = this.items.except(item).map((item: { DOM: { imgWrap: any } }) => item.DOM.imgWrap);
  ...
  closeItem(item: {
    DOM: {
      el: { classList: { remove: (arg0: string) => void } };
      closeCtrl: gsap.TweenTarget;
      titleTexts: gsap.TweenTarget;
      subtitleHeader: gsap.TweenTarget;
      imgWrap: gsap.TweenTarget;
    ...
    gsap
      ...
      .to(
        this.items.except(item).map((item: { DOM: { imgWrap: any } }) => item.DOM.imgWrap),
        {
          duration: 0.7,
          ease: 'power4.inOut',
          startAt: { x: '+=20%' },
